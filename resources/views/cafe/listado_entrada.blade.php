...
<div class="col-12">
    <div class="card">
            ...
            <div class="pt-4">
            <button class=" btn btn-info "  data-toggle="modal" data-target="#modal-lg"><i class="fa fa-plus"></i> Nueva Entrada</button>
            </div>
            </div>
    ...
    @include('cafe.modals.registro_entrada')
    @include('cafe.modals.edicion_entrada')
    ...
    <script>
        ...
        $(document).on('click','.edit',function(){
            ...
            if(@json($session)!=1){
                ...
                $(".frme_0001 input[name='c01']").prop("disabled",true);
                $(".frme_0001 select[name='c03']").prop("disabled",true);
                $(".frme_0001 select[name='c04']").prop("disabled",true);
                $(".frme_0001 select[name='c06']").prop("disabled",true);
                $(".frme_0001 input[name='c07']").prop("disabled",true);
                $(".frme_0001 input[name='c08']").prop("disabled",true);
                $(".frme_0001 select[name='c09']").prop("disabled",true);
                $(".frme_0001 input[name='c11']").prop("disabled",true);
				$(".frme_0001 input[name='c12']").prop("disabled",true);
                $(".frme_0001 input[name='c13']").prop("disabled",true);
                $(".frme_0001 input[name='c14']").prop("disabled",true);
                $(".frme_0001 input[name='c15']").prop("disabled",true);
                $(".frme_0001 input[name='c16']").prop("disabled",true);
                $(".frme_0001 input[name='c17']").prop("disabled",true);
            }

            $.get('{{ asset("cafe/entrada/detalle") }}/'+id,function(d){
                ...
                $(".frme_0001 input[name='usuario']").val(@json($session));
            },'json');
        });
		...
		$('.frm_0001 .calc').keyup(function(){
			...
			p3.val(p1.val().replace('.','')-p2.val().replace('.',''));
			p5.val(p3.val().replace('.','')-p4.val().replace('.',''));
		})
        ...
        $(document).on('click','.delete',function(){
            if(@json($session)==1){
                if(confirm('Estas seguro que deseas eliminar el registro seleccionado?')){
                    var id= $(this).attr('data-id');
                    window.location='{{ asset("cafe/entrada/eliminar") }}/'+id;
                }
            }else{
        ...
        $(document).ready(function(){
           ...
           var tabla= $('#example').DataTable({
                ...
                order: false,
                ajax: { 
                    url: '{{ asset("cafe/listado/entradas/data") }}',
                    dataSrc: 'data',
                    type:'post'
                },
                columnDefs: [
			...
			if(fecha_inicial!=''&&fecha_final!=''){
				tabla.ajax.url('{{ route("cafe.entradas.data") }}?fecha_inicial='+fecha_inicial+'&fecha_final='+fecha_final+'&modo=0').load();
        ...
        $('.prov').change(function(){
            ...
            if(fecha_inicial!=''&&fecha_final!=''){
				tabla.ajax.url('{{ route("cafe.entradas.data") }}?fecha_inicial='+fecha_inicial+'&fecha_final='+fecha_final+'&modo=0'+'&id_prov='+id).load();
