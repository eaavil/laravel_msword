...
$codigo_orden=Str::random(40);//csrf_field()
$tamaño="287%";
if($movil==1){
$tamaño="100%";
}
...
<div class="gestionar_ordenes  container-fluid">
    <table id="2" width="100%"  class="origen texto4" border=1 cellspacing="0" cellpadding="0"  >
         <thead >
            <tr border="1"  >
               <th  width="2%"><h4><STRONG>#</STRONG></h4> </th>
              
               <div  >
                   <th class="principal_col"><h4><STRONG>PAT.</STRONG></h4> </th>
                   <th class="principal_col"><h4><STRONG>T.</STRONG></h4> </th>
                ...
                @if($rows->tiempo_total!="")
                  ...
                  <td> <h4>{{str_replace('OT-','',$rows->numero)}} </h4>
                   ...
                   <td> <h4>
                       ...
                       {{substr(str_replace('despacho','',$rows->patente),0,6)}}</h4>
                        
                   </td>
                   <td> <h4>{{$rows->tiempo_total}} </h4>
                  ...
                  @if($rows->estado==1)
                        <input class="estado almacenar_base_datos" type="checkbox" checked="checked" style="width: 35px; height:30px; padding:0px;" id_orden={{$rows->id}} >
                  @else
                        <input class="estado almacenar_base_datos" type="checkbox" style="width: 35px; height:30px; padding:0px;" id_orden={{$rows->id}}>
                  @endif
                        <a role="button" class="mostrar_listas d:none" onclick="gestionar_listas({{$rows->id}})" style='font-size:35px;color:white;display:none'><i 
                  ...
                  @endif
                  @if($id_usuario==1)
                  <a role="button" class="print" data-id="{{$rows->id}}"><i  style='font-size:40px;color:white' class="fas fa-tablet-alt"></i></a>
                  <a role="button" class="print_escritorio" data-id="{{$rows->id}}"><i  style='font-size:35px;color:white' class="fas fa-desktop"></i></a>
            ...
            $.each(v.ordenes_detalles, function(i,l){
               ...
               l.id_elemento==242){
                   ...
                   if(nombre=="sin descripcion"){
                    ...
                    contenido_tabla+=linea+ `<label for="name" style="width:`+@json($tamaño)+`"><h5>`+nombre+" "+parte+`</h5></label>
   ...
   $('.origen tbody').append(`
                 ...
                 <tr>
                  ...
                  <input type="text" class="dias"  
                    minlength="4" maxlength="1" size="1">
                   <input type="text" class="val_dia"  
                    minlength="4" maxlength="5" size="5">
                  <input type="text" class="descuentos"  name="descuentos" 
                    minlength="4" maxlength="5" size="5">
                  </td>
                  </tr>
                 <tr>
...
if(@json($id_usuario)==1){
       $.post('{{ asset("/registrar/base_datos") }}',{token:@json($codigo_nomina),total:$('.total').val(),sueldo:$('.sueldo').val(),empleado:$(".empleado").val(),ordenes:ordenes,tipo_campo:"nomina",descuentos:$(".descuentos").val(),observaciones:$(".observaciones").val()},function(result){
