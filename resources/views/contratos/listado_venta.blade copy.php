...
<div class="col-12">
    <div class="card">
            <!-- /.card-header -->
            <div class="card-body">
               <button  class="btn btn-secondary" style="float:left" data-toggle="modal" data-target="#modal-2"><i class="fas fa-file"></i> Reporte corte compras/ventas</button>

                <button class="btn btn-info nuevo" style="float:right" data-toggle="modal" data-target="#modal-lg"><i class="fa fa-plus"></i> Contrato de Venta</button>
    ...
    @if(\App\Http\Controllers\AuthController::checkAccessModule('contratos.venta.registrar',session('role_id')))
        @include('contratos.003_contratos.modals.registro')
        @include('contratos.003_contratos.modals.comentario')
    @endif
...
var item=1;
	...
	$(document).on('change',"[type='text']",function (evt) {
         ...
         var total2=1;
            $.each($('.origen tbody tr'),function(i,v){
                ...
                var total=parseInt(( temp * temp2)+(temp*temp2*$(this).find('select[name="aumento[]"]').val()));
                var total_uf=parseFloat(total/@json($uf)).toFixed(2);
                total = total.toString().split('').reverse().join('').replace(/(?=\d*\.?)(\d{3})/g,'$1.');
                total = total.split('').reverse().join('').replace(/^[\.]/,'');//formatear con punto
				$(this).find('input[name="total[]"]').val(total);//mostrar en input
                $(this).find('input[name="uf[]"]').val(total_uf);
               //$(this).find('input[name="uf[]"]').val(number_format(total_uf, 2));
                }else{
                    $(this).find('input[name="total[]"]').val($(this).find('input[name="valor[]"]').val());
                }
    ...
    var tabla = $('#main').DataTable({
        ...
        pageLength: 20,
        ajax: {
            url: '{{ route("contratos.ventas.data") }}',
            dataSrc: 'data',
            type:'post'
        },
        columns: [
            ...
            {"data":null,name:'pendiente_cotizacion', "render": function (data){
                    if(data.pendiente_cotizacion==0){
                        return '<input class="destacar_cotizacion" type="checkbox" style="width: 26px; height:26px; padding:0px; " data-id="'+data.contrato_id+'" >';
                    }else{
                        return '<input class="destacar_cotizacion" type="checkbox" style="width: 26px; height:26px; padding:0px;"  checked  data-id="'+data.contrato_id+'" >';
                    }
                }
            },
            {"data":null,name:'pendiente_factura', "render": function (data){
                                    if(data.pendiente_factura==0){
                                        return '<input class="destacar_factura" type="checkbox" style="width: 26px; height:26px; padding:0px; " data-id="'+data.contrato_id+'" >';
                                    }else{
                                        return '<input class="destacar_factura" type="checkbox" style="width: 26px; height:26px; padding:0px;"  checked  data-id="'+data.contrato_id+'" >';
                                    }
                        }
            ...
            },
            {
                ...
                render: function (data) {
                    ...
                    else{html= 'Sin Cliente Asociado';}
                    return html;
                }
            },
            {data:'orden_trabajo',name:'orden_trabajo'},
            {
            ...
            },
            {
                ...
                "render": function (data) {
                        ...
                        if(parseInt(data.contrato_estado)>=2){
                            html = '<button style="width: 26px; height:26px; padding:0px" data-id="'+data.contrato_id+'" class="btn btn-danger terminar "  title="deshabilitar Contrato"><i class="fa fa-times"></i></button>';
                        
                        }else{
                            html = '<button style="width: 26px; height:26px; padding:0px" data-id="'+data.contrato_id+'" class="btn btn-success habilitar" data-orden="'+data.orden_trabajo+'"  title="habilitar Contrato"><i class="fa fa-check"></i></button>';
                        }
                        if(parseInt(data.contrato_estado)>=2&&parseInt(data.tipo_servicio)==1&&data.numero.split('-').length==2){
                            html = '<button style="width: 26px; height:26px; padding:0px" data-id="'+data.contrato_id+'" class="btn btn-danger terminar "  title="Devolver Contrato"><i class="fa fa-times"></i></button> \
                            <button style="width: 26px; height:26px; padding:0px" data-target="#modal-lg" data-toggle="modal" data-id="'+data.contrato_id+'" class="btn btn-success anexo" title="Agregar anexo"><i class="fa fa-plus"></i></button>';
                        }
                        return html+' '+'<button style="width: 26px; height:26px; padding:0px" data-target="#modal-lg" data-toggle="modal"  data-id="'+data.contrato_id+'" data-orden="'+data.id_orden_trabajo+'" data-estado="'+data.contrato_estado+'"  data-cliente="'+data.id_catalogo_empresa_cliente+'" class="btn btn-info edit"><i class="fa fa-pencil-alt"></i></button> \
                            <button style="width: 26px; height:26px; padding:0px" class="btn btn-danger delete" data-id="'+data.contrato_id+'"><i class="fa fa-trash"></i></button>\
                                <button style="width: 26px; height:26px; padding:0px" class="btn btn-success print" data-id="'+data.contrato_id+'"><i class="fa fa-print"></i></button>';
                }
            }
