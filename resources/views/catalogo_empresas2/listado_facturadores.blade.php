...
<div class="col-12">
    ...
    @if(\App\Http\Controllers\AuthController::checkAccessModule('catalogo.registrar.facturadores',session('role_id')))
        @include('catalogo_empresas.000_catalogo_empresas.modals.registro')
    @endif
    @if(\App\Http\Controllers\AuthController::checkAccessModule('catalogo.actualizar.facturadores',session('role_id')))
    ...
    <script>
        ...
        $('.edit').click(function(){
            ...
            $.get('{{ asset("catalogo/detalle") }}/'+id,function(d){
                ...
                $(".frme_0001 select[name='c09']").val(d.id_banco).trigger('change');
                $(".frme_0001 input[name='c10']").val(d.numero_cuenta);
                $(".frme_0001 select[name='c11']").val(d.tipo_cuenta).trigger('change');
