...
function number_format (number, decimals, dec_point, thousands_sep) {
    ...
    return s.join(dec);
		...
		$('.prov').change(function(){
			...
			if(id!=-1){
				tabla.ajax.url('{{ asset("liquidaciones/salida/liquidada/data") }}?id_prov='+id).load();
			}else{
				tabla.ajax.url('{{ asset("liquidaciones/salida/liquidada/data") }}').load();
			}
			if(fecha_inicial!=''&&fecha_final!=''){
				tabla.ajax.url('{{ route("liquidaciones.salidas.liquidadas.data") }}?fecha_inicial='+fecha_inicial+'&fecha_final='+fecha_final+'&modo=0'+'&id_prov='+id).load();
		...
		var tabla = $('#main').DataTable({
            ...
            paging: true,
            ajax: {
                url: '{{ asset("liquidaciones/salida/liquidada/data") }}',
                dataSrc: `data`,
                type:'post'
            },
            columns: [
                {
                    ...
                    render: function (data){
                        return `
							...
							% Retencion Fuente: `+data.porcentaje_retencion+`<br/>
							Retencion Fuente: `+data.valor_retencion_fuente+`<br/>
                ...
                {
                    data: null,name:'000_catalogo_empresas.nombre',name:'numero_ticket',
                    render: function (data){
                        return `
							<h3>`+data.numero_ticket+`</h3>
							<p>Cliente: `+data.cliente+`<br/>
							Nit: `+data.cliente_nit+`<br/>
                ...
                {
                    data: null,name:'id_liquidacion',
                    render: function (data){
                        return `
							<button type="button" class="btn btn-danger eliminar" data-id="`+data.id_liquidacion+`"><i class="fa fa-trash" aria-hidden="true"></i></button>
						`;
                    }
                }
            ],
        ...
        $(document).on('click','.eliminar',function(){
            if(confirm('Estas seguro que deseas eliminar el registro seleccionado?')){
                var id= $(this).attr('data-id');
                window.location='{{ asset("liquidaciones/salida/eliminar") }}/'+id;
            }
        });
