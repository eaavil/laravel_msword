...
<script>
    ...
    $(document).ready(function(){
		...
		$('.prov').change(function(){
			...
			if(id!=-1){
				tabla.ajax.url('{{ asset("liquidaciones/entrada/contratos/data") }}?id_prov='+id).load();
			}else{
				tabla.ajax.url('{{ asset("liquidaciones/entrada/contratos/data") }}').load();
			}
            if(fecha_inicial!=''&&fecha_final!=''){
				tabla.ajax.url('{{ route("liquidaciones.contratos.pendientes.data") }}?fecha_inicial='+fecha_inicial+'&fecha_final='+fecha_final+'&modo=0'+'&id_prov='+id).load();
		...
		var tabla = $('#main').DataTable({
			...
			pageLength: 20,
            ajax: {
                url: '{{ asset("liquidaciones/entrada/contratos/data") }}',
                dataSrc: 'data',
                type:'post',
            },
            columns: [
