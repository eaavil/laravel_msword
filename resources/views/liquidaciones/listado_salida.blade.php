...
<div class="col-12">
    <div class="card">
            ...
            <div class="mx-2">
            <br>
                <a target="__blank" href="{{ route('liquidaciones.salidas.reporte.resumen') }}"  class="btn btn-secondary" style="float:left"><i class="fa fa-file"></i> Generar Resumen</a>
            </div>  	
            ...
            <div class=" col-3 float-right">  
			  ...
			  <table id="main" class="table table-bordered table-striped" width="100%">
                <thead>
                <tr>
                  <th width="20%">Numero Entrada</th>
                  <th width="20%">Contacto</th>
                  <th width="20%">Detalles Carga</th>
    ...
    return s.join(dec);
        ...
        $(document).on('click','.liquidar',function(){
            ...
            $.get('{{ asset("liquidaciones/salida/detalle") }}/'+id,function(d){
				...
				$.get('{{ asset("liquidaciones/salida/contratos/cliente") }}/'+d[0].cliente.id,function(dx){
                ...
                let html = `
                    <div class="row">
                        <div class="col-4">
                            ...
                            Nombre: `+d[0].cliente.nombre+`<br>
                            NIT: `+d[0].cliente.nit+`<br>
		...
		$(document).on('click','.ver_liquidaciones',function(){
            ...
            $.get('{{ asset("liquidaciones/salida/detalle") }}/'+id,function(d){
				...
				var tablax = $('#mainxc').DataTable({
					...
					scrollX:true,
					ajax: {
						url: '{{ asset("liquidaciones/salida/listado/detalle/data") }}/'+id,
						dataSrc: 'data'
					},
					columns: [
						{
		...
		$('.prov').change(function(){
			...
			if(id!=-1){
				...
				tabla.ajax.url('{{ asset("liquidaciones/salida/listado/data") }}').load();
			}
            if(fecha_inicial!=''&&fecha_final!=''){
				tabla.ajax.url('{{ route("liquidaciones.salidas.listadox") }}?fecha_inicial='+fecha_inicial+'&fecha_final='+fecha_final+'&modo=0'+'&id_prov='+id).load();
		...
		var tabla = $('#main').DataTable({
            serverSide: true,
            ajax: {
                url: '{{ asset("liquidaciones/salida/listado/data") }}',
                dataSrc: 'data',
                type:'post'
            },
            columns: [
                {
                    ...
                    render: function (data){
                        return `
                                ...
                                Fecha:<br>
                                `+data.fecha_ticket+`<br><br>
                                Actualizada el:<br>
                                `+data.entradas_salidas_cafe_updated_at+`<br>
								Tipo de Cafe:<br><br>
								`+data.tipo_cafe+`
								`;
                    }
                },
                {
                    ...
                    render: function (data){
						let html = `<h5>Cliente</h5>`;

                                if(data.id_catalogo_cliente!=null){
                                    html +=data.nombre+`<br>`+data.nit+`-`+data.digito_verificacion_nit+`<br><br>`;
                                }
								return html;
                    }
                },
                ...
                },
                {
                    ...
                    render: function (data){
                             return  `
							<button type="button" class="btn btn-primary liquidar" data-tipo_cafe="`+data.tipo_cafe+`" data-id="`+data.entradas_salidas_cafe_id+`" data-toggle="modal" data-target="#modal-lg"><i class="fa fa-money-bill" aria-hidden="true"></i></button>
						     `;
