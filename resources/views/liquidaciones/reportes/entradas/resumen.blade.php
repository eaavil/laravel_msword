...
<table width="100%">
            ...
            <h2>LISTADO RESUMEN POR PROVEEDORES</h2>
            </td><td><td ></td></td>
            <td align="right">
            <p >
...
<br>
<input id="myInput" class="oculto" type="text" placeholder="Filtrar" style="float:right; margin:1%">
<table class="paleBlueRows" width="100%">
    ...
    @foreach($data as $registros)
	  @if(($registros->kilos_pendientes_scontrato+$registros->kilos_pendientes+$registros->kilos_pendientes_ccontrato+$registros->kilos_liquidados+$registros->total_giros+($registros->total_giros-$registros->total_valor_liquidaciones)+($registros->valor_en_kilos)+($registros->valor_en_kilos-($registros->total_giros-$registros->total_valor_liquidaciones)))!=0)
        <tr>
            <td><a  href="{{route('liquidaciones.entradas.reporte.detalle')}}?id={{$registros->id}}&&proveedor={{ $registros->nombre }}" target="__blank" style="float:left">{{ $registros->nombre }}</a></td>
            ...
            <td>{{ number_format($registros->kilos_pendientes,0,',','.') }}</td>
            <td>{{ number_format($registros->valor_en_kilos,0,',','.') }}</td>
            <td>{{ number_format(($registros->total_giros-$registros->total_valor_liquidaciones)-$registros->valor_en_kilos,0,',','.') }}</td>
        </tr>
        @php
            ...
            $tot_h+=($registros->total_giros-$registros->total_valor_liquidaciones)-$registros->valor_en_kilos;
        @endphp
	  @endif
